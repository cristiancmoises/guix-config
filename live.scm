...
;;              flip/page-flip options. 

(define my-xlibre-config
  (xlibre-configuration
    (modules (list xlibre-video-amdgpu xlibre-video-intel xlibre-input-libinput))
    (drivers '("amdgpu" "intel"))
...
(kernel-arguments
 '( ;; ─── Boot and General ─────────────────────────────────────────────
   ...
   "module.sig_enforce=1"              ; Enforce signed kernel modules
   "kptr_restrict=2"                   ; Hide kernel pointers from /proc
   ...
   "sched_yield_type=2"                ; Aggressive yield for CFS
   ...
   "mitigations=auto"                  ; Auto-apply CPU security mitigations
   "spec_store_bypass_disable=prctl"   ; Spectre v4 mitigation
   "mce=1"                             ; Enable Machine Check Exception

   ;; ─── USB Security ───────────────────────────────────────────────
   "usbcore.authorized_default=0"      ; Disable auto-authorizing USB devices (anti-BadUSB)

   ;; ─── Networking Optimizations ───────────────────────────────────
   "tcp_congestion_control=bbr"        ; Use BBR for efficient TCP
   "net.core.default_qdisc=fq_codel"   ; Use fq_codel for fair queueing
   "net.ipv4.tcp_fq_codel_quantum=1000"
   "net.ipv4.tcp_fq_codel_target=5000"
   ...
   "net.ipv4.tcp_sack=1"               ; Enable TCP Selective Acks
   "net.ipv4.tcp_early_retrans=3"      ; Enable early retransmission
   "net.ipv4.tcp_thin_linear_timeouts=1" ; Optimize timeouts for thin streams
   "ipv6.disable=0"                    ; Keep IPv6 enabled

   ;; ─── AMD GPU (uncomment when running AMD) ───────────────────────
   ;"amdgpu.ppfeaturemask=0xffffffff"   ; Unlock all AMDGPU features
   ...
   ;"amdgpu.dcfeaturemask=0xffffffff"   ; Enable all DC features
   ...
   ;"amdgpu.runpm=1"                    ; Runtime power management
   ;"h264_amf=1"                        ; Enable AMD H.264 encoding

   ;; ─── Intel GPU (uncomment when running Intel) ───────────────────
   ;"i915.enable_psr=1"                 ; Panel Self Refresh for Intel iGPU
   ;"i915.enable_dc=2"                  ; Enable deeper power states
   ...
   "irqaffinity=1-3"                   ; Pin IRQs to CPUs 1-3
   "cpufreq.default_governor=schedutil" ; Use schedutil governor
   "amd_pstate=active"                 ; AMD P-state driver (ignored on Intel)
   "rcu_nocbs=0-3"                      ; Offload RCU callbacks from CPUs

   ;; ─── IOMMU and Virtualization ───────────────────────────────────
   "iommu=pt"                          ; Passthrough mode (works on AMD/Intel)
   "amd_iommu=on"                      ; Enable AMD IOMMU (ignored on Intel)
   ;"intel_iommu=on"                    ; Enable Intel IOMMU (ignored on AMD)

   ;; ─── System Behavior ────────────────────────────────────────────
   "noirqdebug"                        ; Disable IRQ debugging
   "watchdog"                          ; Enable hardware watchdog
   "noreplace-smp"                     ; Prevent SMP code replacement
   "sysrq_always_enabled=1"            ; Enable SysRq key
   "modprobe.blacklist=firewire_core,firewire_ohci,dccp,sctp,rds,tipc" ; Blacklist insecure modules
 ))
  ...
  (users
   (cons*
    (user-account
     ...
     (comment "SecurityOps")
     (group "users")
     (home-directory "/home/securityops")
...
;; ─── Firewall ───────────────────────────────────────────────
;; Strict nftables firewall tuned for privacy + VPN (Mullvad)
;; In live ISO: placeholders are left for user customization.
;; Replace $MULLVADIP with Mullvad server IP(s).
;; Replace $LOCALNETWORK with your LAN range (ex: 192.168.1.0/24).
;; Replace Mullvad DNS (100.64.0.23) if using another provider.
...
(service nftables-service-type
  (nftables-configuration
    (ruleset
      (plain-file
       "nftables.conf"
       "# Live ISO Default nftables Ruleset
flush ruleset
...
table inet filter {
    ...
    chain input {
        ...
        ip saddr { $MULLVADIP } tcp sport 443 ct state established limit rate 4/second accept

        # ICMP essentials
        ip protocol icmp icmp type { echo-request, destination-unreachable, time-exceeded } limit rate 1/second accept
        ip6 nexthdr ipv6-icmp icmpv6 type { nd-neighbor-solicit, nd-router-advert, nd-neighbor-advert, echo-request, destination-unreachable, time-exceeded } limit rate 1/second accept

        # Tor (local only)
        tcp dport { 9050, 9040 } iif \"lo\" limit rate 4/second accept

        # Basic TCP sanity
        tcp flags syn / fin,syn,rst,ack limit rate 12/second accept
        tcp flags fin,psh,urg / fin,psh,urg drop
        tcp flags syn,rst,ack / syn,rst drop
    ...
    chain output {
        ...
        # VPN (WireGuard + DNS + Web)
        udp dport 51820 limit rate 8/second accept
        ip daddr { $MULLVADIP } tcp dport 443 limit rate 4/second accept
        oif \"wg0-mullvad\" { udp dport 53, tcp dport 53 } ip daddr 100.64.0.23 accept
        oif \"wg0-mullvad\" tcp dport 443 limit rate 50/second accept
        oif \"wg0-mullvad\" tcp dport 9418 limit rate 10/second accept

        # Steam (example ranges)
        oif \"wg0-mullvad\" { tcp dport 27015, udp dport 27015, tcp dport 27036, udp dport 27036 } ip daddr { 162.254.192.0/18, 146.66.152.0/21 } accept

        # Torrents
        oif \"wg0-mullvad\" { tcp dport 6881-6890, udp dport 6881-6890 } accept

        # Allow all through VPN as fallback
        oif \"wg0-mullvad\" accept

        # LAN + IPv6 local
        ip daddr { $LOCALNETWORK } accept
...
;; Expose Root Cache for the user Berkeley
    (service shared-cache-service-type
             (shared-cache-configuration
               ...
               (users (list (user-cache (user "securityops"))))))
